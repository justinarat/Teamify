...
Revision ID: 0c91e2b42dfd
Revises: 7b4cc357a403
Create Date: 2024-05-14 18:30:16.961196
...
revision = '0c91e2b42dfd'
down_revision = '7b4cc357a403'
branch_labels = None
depends_on = None
...
def downgrade():
    ...
    with op.batch_alter_table('LobbyTimes', schema=None) as batch_op:
        ...
        batch_op.drop_constraint('fk_lobby_times_lobby', type_='foreignkey', name='fk_lobby_times_lobby')  # Provide name for the constraint
    ...
    with op.batch_alter_table('LobbyTags', schema=None) as batch_op:
        batch_op.add_column(sa.Column('LobbyID', sa.TEXT(), nullable=False))
        batch_op.drop_constraint('fk_lobby_tags_lobby', type_='foreignkey', name='fk_lobby_tags_lobby')  # Provide name for the constraint
        batch_op.create_foreign_key(None, 'Lobby', ['LobbyID'], ['LobbyId'])
        batch_op.alter_column('RowID',
               existing_type=sa.String(),
               nullable=True)
    ...
    with op.batch_alter_table('LobbyPlayers', schema=None) as batch_op:
        batch_op.add_column(sa.Column('LobbyID', sa.TEXT(), nullable=False))
        batch_op.drop_constraint('fk_lobby_players_lobby', type_='foreignkey', name='fk_lobby_players_lobby')  # Provide name for the constraint
        batch_op.create_foreign_key(None, 'Lobby', ['LobbyID'], ['LobbyId'])
        batch_op.alter_column('RowID',
               existing_type=sa.String(),
               nullable=True)
